...
def test_search_qbdata_empty_data():
    ...
    xs = []
    hits = search_qbdata(xs, size=5, foreground_len=4, threshold=5.0)
    assert hits == []
...
def test_search_qbdata_quiet_data():
    ...
    xs = [10] * 500
    hits = search_qbdata(xs, size=20, foreground_len=8, threshold=5.0)
    assert hits == []
...
def test_search_data_empty_checks():
    ...
    hits = search_data(data, checks=[], size=20, foreground_len=8, threshold=5.0)
    assert hits == []
...
def test_search_filepath_with_real_data():
    ...
    hits = search_filepath(test_file, checks, size=210, foreground_len=8, threshold=5.0)
...
def test_search_filepath_accepts_string_path():
    ...
    checks = [(EnBand.MID, Quadrant.B)]
    # Pass as string
    hits = search_filepath(str(test_file), checks, size=210, foreground_len=8, threshold=5.0)
    assert isinstance(hits, list)
